...
void MatrixDeallocate(double** matrix, int numb_rows, int numb_cols)
{
...
int main()
{
	...
	try
	{
		...
		auto cilkStart = std::chrono::system_clock::now();
		...
		auto cilkEnd = std::chrono::system_clock::now();


		//PrintAverageVals(eprocess_type::by_rows, average_vals_in_rows, numb_rows);
		//PrintAverageVals(eprocess_type::by_cols, average_vals_in_cols, numb_cols);

		auto duration1 = std::chrono::duration_cast<std::chrono::microseconds>(cilkEnd - cilkStart);
		std::cout << "Avg time: " << duration1.count() / 1000.0 << "ms" << std::endl;

		//Delete
		MatrixDeallocate(matrix, numb_rows, numb_cols);
		delete[] matrix;
